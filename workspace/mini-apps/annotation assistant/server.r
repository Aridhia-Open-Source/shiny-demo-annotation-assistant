...
server <- function(input, output, session) {
 ...
 # Read and pre-process files
  readText <- reactive({
    ...
    Sys.sleep(1)
    on.exit(progress$close())
    txt <- paste(str_trim(readLines(input$files)), collapse=" ")
  ...
  annotatedFile <- reactive({
    ...
    txt <- findingsInfo()
    m <- matchTable()
    starts <- matchTable()[,c("start")]
    ends <- matchTable()[,c("end")] + 1
    ...
    {
      ploc_table <- aLoc()
      s <- ploc_table[,c("start")]
      e <- ploc_table[,c("end")] + 1
    ...
    annotated <- insert_str(txt,insert = c(unlist(pat_vec[1]),unlist(loc_vec[1]),unlist(lat_vec[1]),unlist(neg_vec[1]),unlist(im_vec[1]),rep("</span>",length(ends))),
                            index = c(unlist(pat_vec[2]),unlist(loc_vec[2]),unlist(lat_vec[2]),unlist(neg_vec[2]),unlist(im_vec[2]),ends))
  ...
  negSent <- reactive({
    ...
    sentList <- sent_detect(txt, endmarks = c('.'))
  ...
  removeNegations <- function( sentRange, negations){
     ...
     for (i in seq(1,nrow(negations),1))
  ...
  observedPathology <- function( pathology, filteredSent){
    ...
    for (i in seq(1,nrow(filteredSent),1))
 ...
 writeTable <- function (df, tablename)
{
   ...
   onoClick <- observeEvent(input$writeDB, {
    ...
    df <- data.frame(Pathology1 = as.numeric(0),
                     Pathology2 = as.numeric(0),
                     Pathology3 = as.numeric(0),
                     Pathology4 = as.numeric(0),
                     Pathology5 = as.numeric(0),
                     Pathology6 = as.numeric(0),
                     Pathology7 = as.numeric(0),
                     Pathology8 = as.numeric(0),
                     Pathology9 = as.numeric(0),
                     Pathology10 = as.numeric(0))
